...
; The destination must positively resolve to a GTA IV game root. This installer deliberately
; does not provide an override for unknown layouts: configuration files in the wrong directory
; are harder to recover than a clear path error.
...
; or attempt to repair third-party binaries and dependencies.
Function VerifyPrerequisites
  IfFileExists "$INSTDIR\plugins\GTAIV.EFLC.FusionFix.asi" fusionFixFound
  MessageBox MB_ICONSTOP "FusionFix was not found at:$\r$\n$INSTDIR\plugins\GTAIV.EFLC.FusionFix.asi$\r$\n$\r$\nInstall and launch a compatible FusionFix Complete Edition setup first. This installer does not ship, update, or repair FusionFix. No files were changed."
  Abort
fusionFixFound:
  IfFileExists "$INSTDIR\vulkan.dll" rendererFound
  MessageBox MB_ICONSTOP "FusionFix is present, but vulkan.dll was not found in the game root.$\r$\n$\r$\nThe shipped core profile selects FusionFix's Vulkan route. Install the intended compatible renderer first, or use a separately prepared native-D3D9 configuration. No files were changed."
  Abort
rendererFound:
FunctionEnd

; Process detection uses tasklist only to prevent the common locked-file/partial-write case.
Function VerifyGameClosed
  nsExec::ExecToStack '"$SYSDIR\tasklist.exe" /FI "$\"IMAGENAME eq GTAIV.exe$\"" /FO CSV /NH'
  Pop $0
  Pop $ProcessOutput
  ; A matching CSV tasklist line begins with a quote followed by GTAIV.exe.
...
Function CreateBackup
  ...
  ; Back up only pre-existing files. A missing backup file means this installer created the target.
  ...
  FileWrite $Handle "GTA IV Core Configuration Installer backup$\r$\n"
...
Section "Uninstall"
  ...
  MessageBox MB_ICONEXCLAMATION|MB_YESNO "Restore the latest core-configuration backup from:$\r$\n$LastBackup$\r$\n$\r$\nChoose Yes only if you want to replace the current core configuration with that backup. If an older installer is also installed, remove installers in reverse installation order." IDYES restoreFiles IDNO finishUninstall
