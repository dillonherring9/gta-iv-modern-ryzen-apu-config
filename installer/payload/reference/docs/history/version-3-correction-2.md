# Version 3.0.3 — Corrective Core Configuration Release

Version 3.0.3 corrects the package boundaries, release traceability, and installer behavior identified after Version 3.0.0. It is not a claim of a newly benchmarked performance result. The underlying FusionFix profile remains a reference configuration for the documented Ryzen 5 PRO 6650U / Radeon 660M context; users must follow the included validation protocol before treating it as a result for their own system.

Version 3.0.2 was the first corrective source tag. Its Windows release workflow stopped before artifact assembly because the Chocolatey-installed NSIS compiler was not added to the runner command path. No Version 3.0.2 release assets were published. Version 3.0.3 retains the corrective source changes, explicitly locates `makensis.exe`, and publishes the first traceable corrective asset set from the successor tag.

## Corrected release contract

| Area | Version 3.0.3 correction |
|---|---|
| Source traceability | This release is generated from its immutable `v3.0.3` Git tag. Its source archive includes `SOURCE_COMMIT.txt`. Historical tags and releases are not rewritten. |
...
| Build disclosure | The repository records the Windows/NSIS release contract in [`BUILD_ENVIRONMENT.md`](BUILD_ENVIRONMENT.md). The workflow now locates the Chocolatey-installed compiler explicitly rather than assuming it is already on `PATH`. |
...
Version 3.0.3 does not delete Version 2, Version 3.0.0, or failed Version 3.0.2 tag history, backups, or uninstall entries. If a prior installer is installed, remove installers in **reverse installation order** after making a complete game-folder backup. The Version 3.0.3 installer uses its own backup root and its uninstaller restores only files it managed.
...
Version 3.0.3 contains no Rockstar, FusionFix, DXVK, GPLAsync, or third-party mod binaries. Credit remains with the original projects and their contributors.
