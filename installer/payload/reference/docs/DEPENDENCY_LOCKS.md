...
Version 3 does not redistribute third-party binaries. This document records the exact public artifacts relevant to the core installer and optional GPLAsync overlay so a configuration result can be reproduced or rejected on evidence. It does **not** claim that Gillian’s Drag-and-Drop Archive installed the listed GPLAsync artifact on every computer: the archive Setup Utility selects a renderer path from local hardware support and settings. Identify the actual local renderer before changing it.[^1]
...
| DXVK GPLAsync | Optional renderer overlay only | `dxvk-gplasync-v2.6.2-1.tar.gz` | `c645d3f2550ff66d0a51ba27a535aaa7351fd087feb7b298e7283cb2a3ee173b` | Required only for `tuned/renderers/dxvk-gplasync-v2.6.2-1.conf`. It is not proof of the archive’s installed renderer. Do not apply that configuration to upstream DXVK, DXVK-Sarek, DXVK 3.0.2, or another GPLAsync version. |
...
Before selecting an overlay, open the game folder’s `dxvk.conf`, save the effective `GTAIV_d3d9.log`, and record the SHA-256 hash of `vulkan.dll`. The presence of `vulkan.dll` alone does not identify stock DXVK, GPLAsync, or another async-capable renderer. Follow [`ARCHIVE_RENDERER_IDENTIFICATION.md`](ARCHIVE_RENDERER_IDENTIFICATION.md) before replacing a working binary.

## Source links

[^1]: [Gillian’s GTA IV Setup Utility](https://github.com/gillian-guide/GTAIVSetupUtility)
