# Version 3.0.5 Dependency Locks

Version 3.0.5 does not redistribute third-party binaries. This document records the exact public artifacts relevant to the core installer and optional GPLAsync overlay so a configuration result can be reproduced or rejected on evidence.
...
A different hash is not automatically malicious, but it is not the artifact described by this Version 3.0.5 contract. Do not call a result comparable to the reference configuration without recording the actual artifact and hash.
