# Version 3.0.4 — Corrective Core Configuration Release

Version 3.0.4 corrects the package boundaries, release traceability, and installer behavior identified after Version 3.0.0. It is not a claim of a newly benchmarked performance result. The underlying FusionFix profile remains a reference configuration for the documented Ryzen 5 PRO 6650U / Radeon 660M context; users must follow the included validation protocol before treating it as a result for their own system.

Version 3.0.2 was the first corrective source tag. Its Windows release workflow stopped before artifact assembly because the Chocolatey-installed NSIS compiler was not added to the runner command path. Version 3.0.3 explicitly located `makensis.exe`, but its workflow then stopped before artifact assembly because the generated installer directory did not exist on the clean Windows runner. No Version 3.0.2 or Version 3.0.3 release assets were published. Version 3.0.4 retains the corrective source changes and creates the output directory before invoking NSIS, so the asset set is built from this successor tag.

## Corrected release contract

| Area | Version 3.0.4 correction |
|---|---|
| Source traceability | This release is generated from its immutable `v3.0.4` Git tag. Its source archive includes `SOURCE_COMMIT.txt`. Historical tags and releases are not rewritten. |
...
Version 3.0.4 does not delete Version 2, Version 3.0.0, or failed Version 3.0.2/3.0.3 tag history, backups, or uninstall entries. If a prior installer is installed, remove installers in **reverse installation order** after making a complete game-folder backup. The Version 3.0.4 installer uses its own backup root and its uninstaller restores only files it managed.
...
Version 3.0.4 contains no Rockstar, FusionFix, DXVK, GPLAsync, or third-party mod binaries. Credit remains with the original projects and their contributors.
